...
def repeated_random(A, max_iter, repn):
    ...
    for iter in range(max_iter):
        ...
        if residue(Sp, A, arr_len, repn) < residue(S, A, arr_len, repn):
            S = Sp
    return S
...
def annealing(A, max_iter, T, repn, S_init = None):
    ...
    for iter in range(max_iter):
        ...
        if residue(Sp, A, arr_len, repn) < residue(S, A, arr_len, repn):
            ...
            exponent = -(residue(Sp,A, arr_len, repn)-residue(S,A, arr_len, repn))/T(iter)
            if np.random.random() < e**(exponent):
                S = Sp
        if residue(S, A, arr_len, repn) < residue(Sbest, A, arr_len, repn):
            Sbest = S
...
start = time.time()
print(estimated_average_residues('prepartition', 25000, 100, 100, 10**12))
end = time.time()
